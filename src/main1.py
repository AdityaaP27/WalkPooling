...
parser.add_argument('--heads', type=int, default=2, help='using multi-heads in the attention link weight encoder')
parser.add_argument('--hidden-channels', type=int, default=32)
parser.add_argument('--batch-size', type=int, default=32)
parser.add_argument('--epoch-num', type=int, default=50)
parser.add_argument('--use-map-loss', type=str2bool, default=True, help='use Mean Average Precision as the loss')
...
# Modify the loss function based on the '--use-map-loss' argument
if args.use_map_loss:
    criterion = None  # Will compute MAP separately
else:
    if args.MSE:
...
def train(loader, epoch):
    ...
    for data in tqdm(loader, desc="train"):
        ...
        # Compute MAP as the loss
        loss = -average_precision_score(label.cpu().detach().numpy(), out.cpu().detach().numpy())
...
Final_Test_MAP_fromloss = 0

Best_Val_fromAUC = 0
Final_Test_MAP_fromAUC = 0

for epoch in range(0, args.epoch_num):
    loss_epoch = train(train_loader, epoch)
    val_map, _, _ = test(val_loader, data_type='val')
    test_map, _, _ = test(test_loader, data_type='test')
    if val_map < Best_Val_fromloss:
        Best_Val_fromloss = val_map
        Final_Test_MAP_fromloss = test_map

    if val_map > Best_Val_fromAUC:
        Best_Val_fromAUC = val_map
        Final_Test_MAP_fromAUC = test_map

    print(f'Epoch: {epoch:03d}, Loss: {loss_epoch:.4f}, Val MAP: {val_map:.4f}, Test MAP: {test_map:.4f}, Picked MAP: {Final_Test_MAP_fromAUC:.4f}')

print(f'From loss: Final Test MAP: {Final_Test_MAP_fromloss:.4f}')
print(f'From AUC: Final Test MAP: {Final_Test_MAP_fromAUC:.4f}')
...
if isinstance(args.log, str):
    ...
    writer.add_scalar('MAP_from_AUC', Final_Test_MAP_fromAUC, record_index)
    writer.add_scalar('MAP_from_loss', Final_Test_MAP_fromloss, record_index)
